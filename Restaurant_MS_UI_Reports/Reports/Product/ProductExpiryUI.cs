...
{
    public partial class ProductExpiryUI : Form
    {
        enum FilterType { Default = 1, Date = 2, Category = 3, Item = 4 }
        FilterType filter = FilterType.Default;
        private UnitOfWork unitOfWork;
        IPagedList<ProductExpiryVM> ppList;
        int PageNo = 1;
        private int SelectedItemId = 0;
        public ProductExpiryUI()
        {
            InitializeComponent();
        ...
        private void LoadProductExpiry()
        {
            try
            {
                using (unitOfWork = new UnitOfWork())
                {
                    ...
                    {
                        ppList = unitOfWork.ItemRspository.GetProductExpiry(DateFrom: dtpFrom.Value, DateTo: dtpTo.Value, pageNo: this.PageNo, pageSize: SharedVariables.PageSize);
                    }
                    else if (filter == FilterType.Item)
                    {
                        ppList = unitOfWork.ItemRspository.GetProductExpiry_Item(ItemId: this.SelectedItemId, pageNo: this.PageNo, pageSize: SharedVariables.PageSize);
                    }
                    else if (filter == FilterType.Category)
                    {
                        ...
                        if (cId > 0)
                        {
                            ppList = unitOfWork.ItemRspository.GetProductExpiry_cat(categoryId: cId, pageNo: this.PageNo, pageSize: SharedVariables.PageSize);
                        }
                        else
                        {
                            ppList = unitOfWork.ItemRspository.GetProductExpiry(pageNo: this.PageNo, pageSize: SharedVariables.PageSize);
                        }
                    }
                    else
                    {
                ...
                {
                    grdData.Rows.Add(i.ItemId, i.ItemName, i.BatchNo, i.BatchExp, i.LastTrDate, i.Quantity);
                }
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            try
            {
                List<ProductExpiryVM> ppList = GetReportData();
                ...
                //    if (d.IsPrint)
                //{
                ProductExpiryViewer v = new ProductExpiryViewer(0, ppList);
                v.Show();
                //}
        ...
        private List<ProductExpiryVM> GetReportData()
        {
            try
            {
                List<ProductExpiryVM> list = new List<ProductExpiryVM>();
                using (unitOfWork = new UnitOfWork())
                {
                    if (filter == FilterType.Date)
                    {
                        list = unitOfWork.ItemRspository.GetProductExpiry_Rpt(DateFrom: dtpFrom.Value, DateTo: dtpTo.Value);
                    }
                    else if (filter == FilterType.Item)
                    {
                        list = unitOfWork.ItemRspository.GetProductExpiry_Item_Rpt(ItemId: this.SelectedItemId);
                    }
                    else if (filter == FilterType.Category)
                    {
                        int cId = Convert.ToInt32(cmbCat.SelectedValue);
                        if (cId > 0)
                        {
                            list = unitOfWork.ItemRspository.GetProductExpiry_cat_Rpt(categoryId: cId);
                        }
                        //else
                        //{
                        //    list = unitOfWork.ItemRspository.GetProductExpiry_Rpt();
                        //}
                    }
                    else
                    {
        ...
        private void btnExcel_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<ProductExpiryVM> Result = GetReportData();
                ProductExpiryRpt_Portrait rpt = new ProductExpiryRpt_Portrait();
                ...
                if (dlgSaveExcel.ShowDialog() == System.Windows.Forms.DialogResult.OK)
                {
                    ...
                    MessageBox.Show("Product Expiry Report Exported Successfully." + Environment.NewLine + "Destination:" + dlgSavePdf.FileName, "Export Success", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
            }
            catch (Exception ex)
            {
