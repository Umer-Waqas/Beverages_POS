            ...
            if (SupplierId > 0)
                using (UnitOfWork uw = new UnitOfWork())
                {
                    data = uw.ExpenseRepository.Query()
                        ...
                        .Where(e => (fromDate.HasValue && toDate.HasValue && DbFunctions.TruncateTime(e.Date) >= DbFunctions.TruncateTime(fromDate.Value) && DbFunctions.TruncateTime(e.Date) <= DbFunctions.TruncateTime(toDate)) || (!fromDate.HasValue || !toDate.HasValue))
